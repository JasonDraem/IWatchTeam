...
- (UIScrollView *)createScrollView{
    ...
    if (!_guideScrollView) {
        ...
        for (NSInteger i = 0; i < 5; i ++) {
            ...
            NSString *path = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"guide_%ld", i + 1] ofType:@"jpg"];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (4 == page.currentPage) {
        ...
        //di
        [IWacthAlertFactory autmicDismisAlertWithTitle:nil withMessage:@"2秒后自动进入或点击进入页面" withTime:1.0 onView:self];
        //});
