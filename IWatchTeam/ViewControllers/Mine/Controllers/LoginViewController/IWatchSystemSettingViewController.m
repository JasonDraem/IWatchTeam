...
- (void)createData{
    ...
    NSArray *sectionOne = @[@"自动离线图片", @"下次启动时清除列表缓存", @"下次启动时清除图片缓存", @"清除缓存",@"关于我们"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (0 == indexPath.section) {
        if (3 == indexPath.row) {
            ...
            [IWacthAlertFactory autmicDismisAlertWithTitle:@"免责声明" withMessage:@"此应用纯属用于技术交流，无任何商业目的，如有数据冲突，请见谅，谢谢" withTime:2.0 onView:self];
        }
    }
        //
    if (1 == indexPath.section && 0 == indexPath.row) {
...
- (void)moveCache{
    ...
    float size = 0;
    NSArray *array = [fileManager contentsOfDirectoryAtPath:str error:nil];
    for (NSInteger i = 0; i<array.count; i++) {
        NSString *fullPath = [str stringByAppendingPathComponent:[array objectAtIndex:i]];
        BOOL isDir;
    ...
    _mess = [NSString stringWithFormat:@"为您缓存:%.2lfM",size];
    
    UIAlertController *AC = [UIAlertController alertControllerWithTitle:@"缓存" message:_mess preferredStyle:UIAlertControllerStyleAlert];
    [AC addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        
    }]];
    
    [AC addAction:[UIAlertAction actionWithTitle:@"清除" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
